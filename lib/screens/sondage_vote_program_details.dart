...
class SondageProgramVoteDetails extends StatelessWidget {
  final String voteId;

  SondageProgramVoteDetails(this.voteId);
  ...
  Future<void> submitVote(String optionId, User? user, BuildContext context) async {
    ...
    final voteDoc = FirebaseFirestore.instance.collection('program_votes_sondage').doc(voteId);

    try {
      var voteSnapshot = await voteDoc.get();
      var voteData = voteSnapshot.data();
      if (voteData == null) {
        // Gestion de cas où les données du vote ne sont pas disponibles
      ...
      if (options == null) {
        // Gestion de cas où les options du vote ne sont pas disponibles
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder(
        stream: FirebaseFirestore.instance.collection('program_votes_sondage').doc(voteId).snapshots(),
